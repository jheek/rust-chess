...
class Chess extends React.Component {
  ...
  render() {
    ...
    const {focusTiles} = this.props;

    const tiles = []
    for (let y = 0; y < 8; y++) {
      for (let x = 0; x < 8; x++) {
        const pos = `${String.fromCharCode(decode.charCodeOffset + x)}${8 - y}`;
        const isSelected = (focusTiles || []).some(p => p == pos);
        const isTarget = targetTile && targetTile.x === x && targetTile.y === y || isSelected;
...
Chess.propTypes = {
  ...
  focusTiles: PropTypes.arrayOf(PropTypes.string)
}
